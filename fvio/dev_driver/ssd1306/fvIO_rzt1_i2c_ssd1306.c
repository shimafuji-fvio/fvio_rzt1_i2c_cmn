...
ST_FVIO_I2C_SSD1306_INFO fvio_i2c_ssd1306_inf;
...
static void fvio_i2c_ssd1306_init( int32_t slot_id )
{
    ...
    //CONFIG設定初期化
    fvio_i2c_ssd1306_inf.TblfvIOCnf[slot_id].cwait = 40;
    fvio_i2c_ssd1306_inf.TblfvIOCnf[slot_id].lwait = 0;
    fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id]      = FVIO_I2C_DMA_STOP;
...
static int32_t fvio_i2c_ssd1306_trg_stop( int32_t slot_id )
{
    ...
    if(fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id] != FVIO_I2C_DMA_STOP ){
...
/***************************************************************************
 * [名称]    :fvio_i2c_ssd1306_wr_ctrl_reg
...
static int32_t fvio_i2c_ssd1306_wr_ctrl_reg( int32_t slot_id, ST_FVIO_I2C_SSD1306_CREG *attr )
{
    ...
    para.dma_num   = 0;
    para.cwait     = fvio_i2c_ssd1306_inf.TblfvIOCnf[slot_id].cwait;
    para.lwait     = fvio_i2c_ssd1306_inf.TblfvIOCnf[slot_id].lwait;
...
static int32_t fvio_i2c_ssd1306_wr_dma( int32_t slot_id, ST_FVIO_I2C_SSD1306_DMA *attr )
{
    ...
    if( attr->trg & (FVIO_CMN_REG_TRG_REP | FVIO_CMN_REG_TRG_SYNC) ){
    	fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id] = FVIO_I2C_DMA_LOOP_PAF;
    }else{
    	fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id] = FVIO_I2C_DMA_BUSY_PAF;
    }
...
static int32_t fvio_i2c_ssd1306_set_cnf( int32_t slot_id, ST_FVIO_I2C_SSD1306_CNF *attr )
{
	fvio_i2c_ssd1306_inf.TblfvIOCnf[slot_id].cwait  = attr->cwait;
	fvio_i2c_ssd1306_inf.TblfvIOCnf[slot_id].lwait  = attr->lwait;
    return 0;
}

/***************************************************************************
 * [名称]    :fvio_i2c_ssd1306_get_cnf
...
static int32_t fvio_i2c_ssd1306_get_cnf( int32_t slot_id, ST_FVIO_I2C_SSD1306_CNF *attr )
{
    attr->cwait = fvio_i2c_ssd1306_inf.TblfvIOCnf[slot_id].cwait;
    attr->lwait = fvio_i2c_ssd1306_inf.TblfvIOCnf[slot_id].lwait;
    return 0;
}

/***************************************************************************
 * [名称]    :fvio_i2c_ssd1306_set_int
...
static int32_t fvio_i2c_ssd1306_set_int( int32_t slot_id, ST_FVIO_I2C_SSD1306_INT *attr )
{
	fvio_i2c_ssd1306_inf.TblfvIOInt[slot_id].paf_callback = attr->paf_callback;
	fvio_i2c_ssd1306_inf.TblfvIOInt[slot_id].pae_callback = attr->pae_callback;
    return 0;
}

/***************************************************************************
 * [名称]    :fvio_i2c_ssd1306_get_bsy
...
static int32_t fvio_i2c_ssd1306_get_bsy( int32_t slot_id )
{
    return fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id];
}

/***************************************************************************
 * [名称]    :fvio_i2c_ssd1306_dma_restart
...
static int32_t fvio_i2c_ssd1306_dma_restart( int32_t slot_id )
{
    ...
    if( ( fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id] & FVIO_I2C_DMA_LOOP_PAF ) == FVIO_I2C_DMA_LOOP_PAF ){
        //次回のDMAを発行(PAF)
...
/***************************************************************************
 * [名称]    :fvio_i2c_ssd1306_isr_pae_func
...
static void fvio_i2c_ssd1306_isr_pae_func( int32_t slot_id )
{
    ...
    //コールバック
    if( fvio_i2c_ssd1306_inf.TblfvIOInt[slot_id].pae_callback != NULL ){
    	fvio_i2c_ssd1306_inf.TblfvIOInt[slot_id].pae_callback(slot_id);
    }

    //busyフラグクリア( LOOPの場合はクリアしない )
    if( fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id] & FVIO_I2C_DMA_BUSY_PAE ){
    	fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id] &= ~FVIO_I2C_DMA_BUSY_PAE;
    }
}

/***************************************************************************
 * [名称]    :fvio_i2c_ssd1306_isr_paf_func
...
static void fvio_i2c_ssd1306_isr_paf_func( int32_t slot_id )
{
    ...
    //コールバック
    if( fvio_i2c_ssd1306_inf.TblfvIOInt[slot_id].paf_callback != NULL ){
    	fvio_i2c_ssd1306_inf.TblfvIOInt[slot_id].paf_callback(slot_id);
    }

    //busyフラグクリア( LOOPの場合はクリアしない )
    if( fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id] & FVIO_I2C_DMA_BUSY_PAF ){
    	fvio_i2c_ssd1306_inf.TblPlugBusy[slot_id] &= ~FVIO_I2C_DMA_BUSY_PAF;
    }
}
